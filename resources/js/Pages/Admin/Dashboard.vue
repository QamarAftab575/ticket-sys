  ...
  <AdminLayout>
    <template #header>
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-4xl font-bold text-gray-900">{{ $t('dashboard') }}</h1>
          <p class="text-gray-600 mt-1">{{ $t('welcome_to_your_admin_panel') }}</p>
        </div>
    ...
    <div class="space-y-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl shadow-lg p-6 border border-blue-200">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-blue-600 text-sm font-semibold uppercase tracking-wide">{{ $t('total_users') }}</p>
              <p class="text-4xl font-bold text-blue-900 mt-3">{{ formatNumber(stats.total_users) }}</p>
              <p class="text-blue-600 text-xs mt-3">
                <span class="font-bold text-green-600">+{{ stats.new_users_today }}</span> {{ $t('new_today') }}
              </p>
            </div>
        ...
        <div class="bg-gradient-to-br from-purple-50 to-purple-100 rounded-2xl shadow-lg p-6 border border-purple-200">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-purple-600 text-sm font-semibold uppercase tracking-wide">{{ $t('workspaces') }}</p>
              <p class="text-4xl font-bold text-purple-900 mt-3">{{ formatNumber(stats.total_workspaces) }}</p>
              <p class="text-purple-600 text-xs mt-3">{{ $t('active_organizations') }}</p>
            </div>
        ...
        <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-2xl shadow-lg p-6 border border-green-200">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-green-600 text-sm font-semibold uppercase tracking-wide">{{ $t('projects') }}</p>
              <p class="text-4xl font-bold text-green-900 mt-3">{{ formatNumber(stats.total_projects) }}</p>
              <p class="text-green-600 text-xs mt-3">{{ $t('across_all_workspaces') }}</p>
            </div>
        ...
        <div class="bg-gradient-to-br from-orange-50 to-orange-100 rounded-2xl shadow-lg p-6 border border-orange-200">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-orange-600 text-sm font-semibold uppercase tracking-wide">{{ $t('tasks') }}</p>
              <p class="text-4xl font-bold text-orange-900 mt-3">{{ formatNumber(stats.total_tasks) }}</p>
              <p class="text-orange-600 text-xs mt-3">{{ $t('total_tasks_created') }}</p>
            </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-blue-500">
          <p class="text-gray-600 text-sm font-medium">{{ $t('new_signups_today') }}</p>
        ...
        <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-indigo-500">
          <p class="text-gray-600 text-sm font-medium">{{ $t('new_signups_this_week') }}</p>
          <p class="text-3xl font-bold text-gray-900 mt-2">{{ stats.new_users_week }}</p>
          <div class="mt-4 text-xs text-gray-500">
            {{ $t('avg') }} {{ (stats.new_users_week / 7).toFixed(1) }} {{ $t('per_day') }}
          </div>
        </div>

        <!-- New Signups Month -->
        <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-cyan-500">
          <p class="text-gray-600 text-sm font-medium">{{ $t('new_signups_this_month') }}</p>
          <p class="text-3xl font-bold text-gray-900 mt-2">{{ stats.new_users_month }}</p>
          <div class="mt-4 text-xs text-gray-500">
            {{ $t('avg') }} {{ (stats.new_users_month / 30).toFixed(1) }} {{ $t('per_day') }}
          </div>
        </div>

        <!-- Active Trials -->
        <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-teal-500">
          <p class="text-gray-600 text-sm font-medium">{{ $t('active_trials') }}</p>
      ...
      <div class="bg-white rounded-2xl shadow-lg p-8 border border-gray-100">
        <div class="mb-8">
          <h2 class="text-2xl font-bold text-gray-900">{{ $t('subscription_overview') }}</h2>
          <p class="text-sm text-gray-600 mt-1">{{ $t('real_time_subscription_and_revenue_metrics') }}</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div class="bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-xl shadow-md p-8 border border-emerald-200 hover:shadow-xl transition-shadow">
            <div class="flex items-start justify-between mb-4">
              <div class="flex-1">
                <p class="text-emerald-700 text-sm font-bold uppercase tracking-wide mb-3">{{ $t('active_subscriptions') }}</p>
          ...
          <div class="bg-gradient-to-br from-rose-50 to-rose-100 rounded-xl shadow-md p-8 border border-rose-200 hover:shadow-xl transition-shadow">
            <div class="flex items-start justify-between mb-4">
              <div class="flex-1">
                <p class="text-rose-700 text-sm font-bold uppercase tracking-wide mb-3">{{ $t('expired_subscriptions') }}</p>
          ...
          <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl shadow-md p-8 border border-blue-200 hover:shadow-xl transition-shadow">
            <div class="flex items-start justify-between mb-4">
              <div class="flex-1">
                <p class="text-blue-700 text-sm font-bold uppercase tracking-wide mb-3">{{ $t('total_subscription_revenue') }}</p>
      ...
      <div class="bg-white rounded-2xl shadow-lg p-8">
        <div class="mb-6">
          <h2 class="text-2xl font-bold text-gray-900">{{ $t('revenue_subscription_trends') }}</h2>
          <p class="text-sm text-gray-600 mt-1">{{ $t('last_12_months_performance_overview') }}</p>
        </div>
        ...
        <div class="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-gray-200">
          <div class="text-center">
            <div class="flex items-center justify-center gap-2 mb-2">
              ...
              <span class="text-sm font-medium text-gray-700">{{ $t('active_subscriptions') }}</span>
            </div>
          ...
          <div class="text-center border-l border-r border-gray-200">
            <div class="flex items-center justify-center gap-2 mb-2">
              ...
              <span class="text-sm font-medium text-gray-700">{{ $t('expired_subscriptions') }}</span>
            </div>
          ...
          <div class="text-center">
            <div class="flex items-center justify-center gap-2 mb-2">
              ...
              <span class="text-sm font-medium text-gray-700">{{ $t('total_revenue') }}</span>
            </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-8">
          <div class="mb-6">
            <h2 class="text-xl font-bold text-gray-900">{{ $t('user_growth') }}</h2>
            <p class="text-sm text-gray-600 mt-1">{{ $t('last_30_days') }}</p>
          </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg p-8">
          <div class="mb-6">
            <h2 class="text-xl font-bold text-gray-900">{{ $t('revenue_trend') }}</h2>
            <p class="text-sm text-gray-600 mt-1">{{ $t('last_12_months') }}</p>
          </div>
          <div class="relative h-80">
      ...
      <div class="bg-white rounded-2xl shadow-lg p-8">
        <div class="mb-8">
          <h2 class="text-xl font-bold text-gray-900">{{ $t('recent_signups') }}</h2>
          <p class="text-sm text-gray-600 mt-1">{{ $t('latest_10_registered_users') }}</p>
        </div>
          ...
          <table class="w-full">
            <thead>
              <tr class="border-b border-gray-200">
                <th class="text-left py-4 px-4 text-sm font-semibold text-gray-700">{{ $t('name') }}</th>
                <th class="text-left py-4 px-4 text-sm font-semibold text-gray-700">{{ $t('email') }}</th>
                <th class="text-left py-4 px-4 text-sm font-semibold text-gray-700">{{ $t('joined') }}</th>
                <th class="text-left py-4 px-4 text-sm font-semibold text-gray-700">{{ $t('status') }}</th>
              </tr>
            </thead>
            <tbody>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Link href="/admin/users" class="group relative bg-gradient-to-br from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 rounded-2xl shadow-lg p-8 text-white transition transform hover:scale-105">
          <div class="flex items-center justify-between">
            <div>
              <h3 class="text-2xl font-bold">{{ $t('manage_users') }}</h3>
              <p class="text-blue-100 mt-2">{{ $t('view_edit_or_suspend_users') }}</p>
            </div>
        ...
        <Link href="/admin/workspaces" class="group relative bg-gradient-to-br from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 rounded-2xl shadow-lg p-8 text-white transition transform hover:scale-105">
          <div class="flex items-center justify-between">
            <div>
              <h3 class="text-2xl font-bold">{{ $t('manage_workspaces') }}</h3>
              <p class="text-purple-100 mt-2">{{ $t('view_and_manage_all_workspaces') }}</p>
            </div>
